...
// @Summary      Kirim pesan dalam sesi percakapan
// @Tags         Conversation
// @Accept       json
// @Produce      json
// @Param        id    path      string                    true  "Session ID"
// @Param        body  body      request.SendMessageRequest  true  "Pesan user"
...
func (h *ConversationHandler) SendMessage(c fiber.Ctx) error {
	...
	if req.Content == "" {
		return response.BadRequest(c, "content wajib diisi")
	}

	result, err := h.convService.SendMessage(c.Context(), userID, sessionID, req)
	if err != nil {
		switch {
